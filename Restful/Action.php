...
class Restful_Action extends Typecho_Widget implements Widget_Interface_Do
{
    ...
    {
        $query = $this->db->select('table.metas.mid', 'table.metas.name', 'table.metas.slug', 'table.metas.description', 'table.metas.count')
            ->from('table.metas')
     ...
     * @return int
     */
    public function getPostViews($cid)
    {
        // 方案1: 从 typecho_contents 表读取 (Joe 主题存储方式)
        // Joe 主题可能使用 views 或 viewsNum 字段
        try {
            ...
            if ($row) {
                ...
                // 检查 viewsNum 字段
                if (isset($row['viewsNum']) && $row['viewsNum'] > 0) {
                    return (int)$row['viewsNum'];
                }
            }
        } catch (Exception $e) {
            // 字段不存在，继续尝试其他方案
        ...
        // 方案2: 从自定义字段读取
        $viewFields = array('views', 'viewsNum', 'view', 'hits');
        ...
        foreach ($rows as $row) {
            if (in_array($row['name'], $viewFields)) {
    ...
    public function linksAction()
    {
        ...
        // 方案1: 尝试从 Joe 主题配置读取友链
        try {
            $joeConfig = $this->db->fetchRow(
                $this->db->select('value')
                    ->from('table.options')
                    ->where('name = ?', 'theme:Joe')
            );

            if ($joeConfig && !empty($joeConfig['value'])) {
                $config = unserialize($joeConfig['value']);
                if (isset($config['JFriends']) && !empty($config['JFriends'])) {
                    $linksData = $this->parseJoeLinks($config['JFriends']);
                }
            }
        } catch (Exception $e) {
            // 配置读取失败，继续尝试其他方案
        ...
        // 方案2: 如果方案1失败，尝试从友链页面的自定义字段读取
        if (empty($linksData)) {
            try {
                // 查找 slug 为 links 的页面
                $linksPage = $this->db->fetchRow(
                    $this->db->select('cid')
                        ->from('table.contents')
                        ->where('type = ?', 'page')
                        ->where('slug = ?', 'links')
                ...
                if ($linksPage) {
                    $cid = $linksPage['cid'];
                    // 查找该页面的 links 或 friends 自定义字段
                    $linksField = $this->db->fetchRow(
                        $this->db->select('str_value')
                            ->from('table.fields')
                            ->where('cid = ?', $cid)
                            ->where('name = ? OR name = ?', 'links', 'friends')
                    );

                    if ($linksField && !empty($linksField['str_value'])) {
                        // 尝试解析 JSON 格式
                        $jsonData = json_decode($linksField['str_value'], true);
                        if (json_last_error() === JSON_ERROR_NONE && is_array($jsonData)) {
                            $linksData = $jsonData;
                        } else {
                            // 如果不是 JSON，尝试解析 Joe 格式
                            $linksData = $this->parseJoeLinks($linksField['str_value']);
                        }
                    }
                }
            } catch (Exception $e) {
        ...
        // 方案3: 如果前两个方案都失败，尝试从 Links 插件表读取
        if (empty($linksData)) {
            try {
                $order = strtolower($this->getParams('order', 'asc'));
                $select = $this->db->select('*')
                    ->from('table.links')
                    ->order('order', $order === 'desc' ? Typecho_Db::SORT_DESC : Typecho_Db::SORT_ASC);
                $result = $this->db->fetchAll($select);
     ...
     * 解析 Joe 主题的友链配置格式
     *
     * @param string $linksString Joe主题友链配置字符串
     * @return array
     */
    private function parseJoeLinks($linksString)
    {
        $result = array();
        ...
        foreach ($lines as $line) {
            ...
            if (count($parts) >= 2) {
                $result[] = array(
                    ...
                    'logo' => isset($parts[2]) ? trim($parts[2]) : '',
    ...
    private function filter($value)
    {
        ...
        // 添加文章标签
        $value['tags'] = $this->getPostTags($value['cid']);

        // 添加文章分类
        $value['categories'] = $this->getPostCategories($value['cid']);

        // 添加阅读量
        $value['views'] = $this->getPostViews($value['cid']);
